      ...
      {(() => {
				...
				const FetchedOptions = ({endpoint}) => {
					...
					if(B.env === 'prod') {
						...
						if(status == 'LOADING' || status == 'IDLE') {
							return <option>Loading values</option>
						}

						if(status == 'ERROR') {
							return <option>Error in component</option>
						}

						return result.map(resultitem => <option value={resultitem.value}>{resultitem.label}</option>)
					} else {
						return <option>Option data from endpoint</option>
					}
        ...
        return (
            ...
            >
							...
							{(() => {
								if(options.model) {
			            return (<><option value="">Blank option</option><GetAll
			              ...
			              {({ loading, error, data }) => {
			                ...
			                const { results } = data;
									...
									return (
										<FetchedOptions endpoint={options.endpoint}>

										</FetchedOptions>
									)
								} else {
									const option_json = JSON.parse(options.optionjson);
									return (option_json.map(option => <option value={option.value}>{option.label}</option>))
								}
